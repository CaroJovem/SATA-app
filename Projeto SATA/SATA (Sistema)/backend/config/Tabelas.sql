...
CREATE TABLE `internacoes` (
  ...
  CONSTRAINT `fk_quarto` FOREIGN KEY (`quarto_id`) REFERENCES `quartos` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `notificacoes` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `tipo` enum('cadastro','estoque_baixo','transacao_financeira') NOT NULL,
  `titulo` varchar(200) NOT NULL,
  `descricao` text NOT NULL,
  ...
  `referencia_tipo` enum('produto','doacao','financeiro','idoso','doador','quarto') DEFAULT NULL,
